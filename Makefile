...
docker_image		:= docker.partner.codes/website:latest
docker_run		:= docker run --rm -p 4242:80 \
				-v `pwd`/container/databases:/partner-codes/databases \
				-v `pwd`/container/config:/partner-codes/config \
				-v `pwd`:/partner-codes/logs \
...
export PARTNER_CODES_CONF_PATH	:= $(example_config)
export NODE_ENV				:= production
export NODE_PATH			:= $(shell pwd)/frontend/src:$(shell pwd)/frontend/node_modules
...
html-docs: develop
	PARTNER_CODES_GENERATE_DOCS=true make functional html-docs-only

html-docs-only:
	cp -f example/config.yaml ./docs/source/_static/example-config.yaml
	cd docs && make html
	rsync -putaoz docs/build/html/ partner_codes/docs/
...
clean:
	...
	@rm -rf partner_codes.egg-info build dist docs/build

run:
	reset && pipenv run partner-codes web --debug --port=$(port) --host=$(host)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
ngrok:
	ngrok start partner_codes

tunnel-run: docker-image

gunicorn:
	gunicorn -c gunicorn.conf -b 0.0.0.0:4242 partner_codes.web:application

notify:
	@osascript -e 'display notification "target completed $(MAKECMDGOALS)" with title "Partner Codes Makefile"'
...
lint:
	flake8 partner_codes tests

aws-tail-logs:
	AWS_PROFILE=develop_devops awslogs get /ecs/partner-codes '.*' --start='30m ago' -w -G  --timestamp
...
db: drop-db
	@echo "CREATE DATABASE IF NOT EXISTS partnercodes" | mysql -uroot
	pipenv run alembic upgrade head

drop-db:
	@echo "DROP DATABASE IF EXISTS partnercodes" | mysql -uroot
