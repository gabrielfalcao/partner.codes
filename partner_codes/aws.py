...
from partner_codes.util import any_of
from partner_codes.logs import get_logger


"""High-level access to Partner Codes's AWS resources based on the config yaml.
...
def resource(name, config=None):
    """auto-loads a session :py:func:`~partner_codes.aws.session`
...
def bucket(name=None):
    """auto-loads a session :py:func:`~partner_codes.aws.resource`
    ...
    def write_profile(self):
        for profile_name, profile_config in conf.get_dict('aws', 'profile').items():
            ...
            if profile_config and not isinstance(profile_config, dict):
                ...
                raise PartnerCodesConfigError(msg.format(profile_name, profile_config))
...
def update_and_write_config_to_home():
    """takes the aws config from PARTNER_CODES_CONF_PATH and safely writes to ~/.aws/config.
...
class PartnerCodesConfigError(RuntimeError):
    pass
