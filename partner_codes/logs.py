...
def determine_log_level():
    """attempts to retrieve the log level from the environment variable ``PARTNER_CODES_LOGLEVEL``
    """
    from_env_vars = os.getenv('PARTNER_CODES_LOGLEVEL')
...
def get_log_filename_from_conf(conf, prefix='partner-codes.%Y-%m-%d.%H-%M'):
...
def get_logger(name, *args, **kw):
    ...
    if name == 'partner_codes' or not name:
        logger.handlers = [handler]

    elif name.startswith('partner_codes.'):
        logger.handlers = []
...
logger = get_logger('partner_codes')
