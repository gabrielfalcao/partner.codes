...
LOOKUP_PATHS = [
    abspath('/etc/partner-codes.yaml'),
    abspath(expanduser('~/.partner-codes.yaml')),
]
...
def get_lookup_paths():
    ...
    initial = os.getenv('PARTNER_CODES_CONF_PATH')
    final = current_path('partner-codes.yaml')
    return [initial] + list(LOOKUP_PATHS) + [final]


def determine_config_path():
    """
    :returns: a :py:class:`pathlib.Path` with the first valid file of the list of config paths (as returned by :py:func:`~partner_codes.conf.get_lookup_paths`)
    """
...
def load(throw=False, fallback=UndefinedFalback):
    """loads the contents of the config at the given PARTNER_CODES_CONF_PATH
    ...
    .. code:: python
       ...
       assert conf.get('dao', 's3', 'bucket_name') == 'partner-codes'
    ...
    data['partnercodes_config_dir'] = str(determine_config_dir())
    return data
...
def get_dict(*args, **kw):
    """alias to :py:func:`~partner_codes.conf.get(*args, **kw, fallback={})`
