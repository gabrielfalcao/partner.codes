...
class ActiveFileRecord(ActiveRecord):
    ...
    def from_object(cls, stream, dao):
        ...
        :param dao: an instance of :py:class:`~partner_codes.dao.files.FileStorage`
        :returns: an :py:class:`~partner_codes.dao.files.ActiveFileRecord`
        """
        try:
...
class FileStorage(Storage):
    ...
    ::
        ...
        from partner_codes.dao.files import FileStorage
    ...
    def new(self, **kw):
        """creates a new :py:class:`~partner_codes.dao.files.ActiveFileRecord` with the given
        keyword-args"""
    ...
    def iter_all(self, namespace):
        ...
        :returns: a generator of 2-item tuples of: :py:class:`~partner_codes.dao.files.ActiveFileRecord` objects and a :py:class:`~str` of its source path in the file-system.
        """
    ...
    def list_all(self, namespace):
        """lists all records under a given namespace, Under the hood it calls :py:meth:`~iter_all` but returns a list of :py:class:`~partner_codes.dao.files.ActiveFileRecord` rather than a 2-item tuple

        :param namespace: :py:class:`~str`
        :returns: a list of :py:class:`~partner_codes.dao.files.ActiveFileRecord` objects with valid data from json files found
        """
    ...
    def retrieve_by(self, **lookup):
        ...
        :returns: a single :py:class:`~partner_codes.dao.files.ActiveFileRecord` object for the first record found with the provided keyword arguments.
    ...
    def delete_all(self, namespace):
        ...
        **Deletes all files under the given namespace!!**
        ...
        .. warning:: this recursive deletes ALL valid json files under the ``root_path`` of this :py:class:`~partner_codes.dao.files.FileStorage`
        """
        result = []
    ...
    def build_persistence_path(self, key_value):
        ...
        :raises: :py:class:`~partner_codes.dao.files.FileStorageError` if no namespace as defined. To remedy that always make sure to use the file-storage like: ``storage.of("my-namespace")`` where the given namespace is a valid directory name.
        """
        key, value = key_value
    ...
    def persist_in_path(self, item, path):
        """persists an :py:class:`~partner_codes.dao.files.ActiveFileRecord` in the given ``path``.

        - creates the parent directory if it doesn't already exist
        - json-serializes the data within :py:class:`~partner_codes.dao.files.ActiveFileRecord` data
        - writes it to the disk at the given ``path``

        :param item: an :py:class:`~partner_codes.dao.files.ActiveFileRecord`
        :param path: a :py:class:`~pathlib.Path`
    ...
    def persist(self, item, key):
        """persists an :py:class:`~partner_codes.dao.files.ActiveFileRecord` by the given key.
    ...
    def delete(self, item, key):
        """deletes an :py:class:`~partner_codes.dao.files.ActiveFileRecord` by the given key.
