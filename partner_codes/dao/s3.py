...
from partner_codes import aws
from partner_codes import conf
from partner_codes.dao.base import Storage
from partner_codes.dao.base import ActiveRecord
...
class ActiveS3Record(ActiveRecord):
    """Subclass of
    :py:class:`~partner_codes.dao.base.ActiveRecord` with extra
    ...
    def from_object(cls, s3_object, dao):
        ...
        :param dao: an instance of :py:class:`~partner_codes.dao.files.S3Storage`
        :returns: an :py:class:`~partner_codes.dao.files.ActiveS3Record`
        """
        try:
...
class S3StorageError(Exception):
    ...
    :py:class:`~partner_codes.dao.base.Storage` without
...
class S3Storage(Storage):
    ...
    :py:class:`~partner_codes.dao.base.Storage`, is simple
    ...
    ::
        ...
        from partner_codes.dao.files import S3Storage

        dao = S3Storage("bucket-name")
        person1 = dao.of("users").new(id=1, email='foo@bar.com')
        dao.persist(person1, 'id')

    this create the key: ``partner-codes/users/id/356a192b7913b04c54574d18c28d46e6395428ab``

    because ``SHA1("1") == "356a192b7913b04c54574d18c28d46e6395428ab"``

    and the content of ``s3://bucket-name/partner-codes/users/id/356a192b7913b04c54574d18c28d46e6395428ab``
    would be:
    ...
    def initialize(self, bucket_name=None, namespace='', fallback_prefix='partner-codes'):
        if not bucket_name:
            bucket_name = determine_bucket_name()
    ...
    def new(self, **kw):
        """creates a new :py:class:`~partner_codes.dao.files.ActiveS3Record` with the given
        keyword-args"""
    ...
    def iter_all(self, namespace=''):
        ...
        :returns: a generator of 2-item tuples of: :py:class:`~partner_codes.dao.files.ActiveS3Record` objects and a :py:class:`~str` of its source path in the file-system.
        """
    ...
    def list_all(self, namespace=''):
        """lists all records under a given namespace, Under the hood it calls :py:meth:`~iter_all` but returns a list of :py:class:`~partner_codes.dao.files.ActiveS3Record` rather than a 2-item tuple

        :param namespace: :py:class:`~str`
        :returns: a list of :py:class:`~partner_codes.dao.files.ActiveS3Record` objects with valid data from json files found
        """
    ...
    def retrieve_by(self, **lookup):
        ...
        :returns: a single :py:class:`~partner_codes.dao.files.ActiveS3Record` object for the first record found with the provided keyword arguments.
    ...
    def delete_all(self, namespace):
        ...
        **Deletes all files under the given namespace!!**
        ...
        .. warning:: this recursive deletes ALL valid json files under the ``root_path`` of this :py:class:`~partner_codes.dao.files.S3Storage`
        """
        namespace = self.ensure_prefixed(namespace)
        for s3_object in self.bucket.objects.filter(Prefix=namespace):
    ...
    def build_persistence_path(self, key_value):
        ...
        :raises: :py:class:`~partner_codes.dao.files.S3StorageError` if no namespace as defined. To remedy that always make sure to use the file-storage like: ``storage.of("my-namespace")`` where the given namespace is a valid directory name.
    ...
    def persist_in_path(self, item, path):
        """persists an :py:class:`~partner_codes.dao.files.ActiveS3Record` in the given ``path``.

        - creates the parent directory if it doesn't already exist
        - json-serializes the data within :py:class:`~partner_codes.dao.files.ActiveS3Record` data
        - writes it to the disk at the given ``path``

        :param item: an :py:class:`~partner_codes.dao.files.ActiveS3Record`
        :param path: a :py:class:`~pathlib.PurePath`
    ...
    def persist(self, item, key):
        """persists an :py:class:`~partner_codes.dao.files.ActiveS3Record` by the given key.
    ...
    def delete(self, item, key):
        """deletes an :py:class:`~partner_codes.dao.files.ActiveS3Record` by the given key.
